...
pub trait SupervisionStrategy {
    async fn handle_failure(&mut self, master: Id, puppet: Id, err: PuppeterError);
}
...
impl SupervisionStrategy for OneForAll {
    async fn handle_failure(
        ...
        async fn restart_all_children_recursively(actor_id: Id, puppeter: &Puppeter, master: Id) {
            if let Some(service_address) = puppeter.get_command_address_by_id(&actor_id) {
                service_address
                    .send_command(crate::prelude::ServiceCommand::RequestRestart { sender: master })
                    .await
                    .unwrap();
            }
            let child_puppets = puppeter.get_puppets_by_id(actor_id);
